...
static GtkWidget *grid;  // Объявляем grid как глобальную переменную, чтобы
                         // иметь к ней доступ из разных функций
...
void add_staff_to_table(int id, const char *name, int passport, int snils, const char *position, int salary,
                        const char *status, const char *emp_date, int phone_num, int inn);
...
void add_order_to_table(int id, const char *customer, const char *name, int value, int amount, int sum,
                        const char *pay_type, const char *date);
...
static void on_add_staff_clicked(GtkWidget *widget, gpointer user_data) {
    ...
    if (name && *name != '\0' && passport_text && *passport_text != '\0' && snils_text &&
        *snils_text != '\0' && position && *position != '\0' && status && *status != '\0' && emp_date &&
        *emp_date != '\0' && phone_num_text && *phone_num_text != '\0' && inn_text && *inn_text != '\0') {
...
static void choose_subsystem_window(GtkApplication *app, gpointer subsystem_data) {
    ...
    orders_radio_button =
        gtk_radio_button_new_with_label_from_widget(GTK_RADIO_BUTTON(personnel_radio_button), "Заказы");
    gtk_grid_attach(GTK_GRID(grid), orders_radio_button, 0, 2, 1, 1);

    warehouse_radio_button =
        gtk_radio_button_new_with_label_from_widget(GTK_RADIO_BUTTON(orders_radio_button), "Склад");
    ...
    gtk_grid_attach(GTK_GRID(grid), button_next, 0, 4, 1,
                    1);  // Сохраняем координаты как 0, 4, 1, 1
...
static void choose_action_window_staff(GtkApplication *app, gpointer subsystem_data) {
    ...
    add_radio_button =
        gtk_radio_button_new_with_label_from_widget(GTK_RADIO_BUTTON(show_radio_button), "Добавить данные");
    gtk_grid_attach(GTK_GRID(grid), add_radio_button, 0, 2, 1, 1);

    delete_radio_button =
        gtk_radio_button_new_with_label_from_widget(GTK_RADIO_BUTTON(add_radio_button), "Удалить данные");
    gtk_grid_attach(GTK_GRID(grid), delete_radio_button, 0, 3, 1, 1);

    end_radio_button =
        gtk_radio_button_new_with_label_from_widget(GTK_RADIO_BUTTON(delete_radio_button), "Завершить");
    ...
    gtk_grid_attach(GTK_GRID(grid), button_next, 0, 7, 1,
                    1);  // Сохраняем координаты как 0, 4, 1, 1
...
static void show_staff_in_window(GtkApplication *app, GtkWidget *widget, gpointer user_data) {
    ...
    GtkListStore *store =
        gtk_list_store_new(10, G_TYPE_INT, G_TYPE_STRING, G_TYPE_INT, G_TYPE_INT, G_TYPE_STRING, G_TYPE_INT,
    ...
    // Заполняем модель данными из базы данных, используя функцию
...
void select_from_staff_table(GtkListStore *store) {
    ...
    while (step == SQLITE_ROW) {
        ...
        gtk_list_store_set(store, &iter, 0, id, 1, name,  // ФИО
                           2, passport,                   // Паспорт
                           3, snils,                      // СНИЛС
...
static void show_orders_in_window(GtkApplication *app, GtkWidget *widget, gpointer order_data) {
    ...
    GtkTreeView *tree_view = GTK_TREE_VIEW(gtk_tree_view_new());
    GtkCellRenderer *renderer = gtk_cell_renderer_text_new();
    GtkListStore *store = gtk_list_store_new(8, G_TYPE_INT, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_INT,
    ...
    gtk_tree_view_insert_column_with_attributes(tree_view, -1, "Наименование товара", renderer, "text", 2,
                                                NULL);
    gtk_tree_view_insert_column_with_attributes(tree_view, -1, "Стоимость одного товара", renderer, "text", 3,
                                                NULL);
...
void select_from_orders_table(GtkListStore *store) {
    ...
    while (step == SQLITE_ROW) {
        ...
        GtkTreeIter iter;
        gtk_list_store_append(store, &iter);
        gtk_list_store_set(store, &iter, 0, id, 1, customer, 2, name, 3, value, 4, amount, 5, sum, 6,
                           pay_type, 7, date, -1);
        step = sqlite3_step(res);
    }
...
void select_from_materials_table(GtkListStore *store) {
    ...
    while (step == SQLITE_ROW) {
        ...
        GtkTreeIter iter;
        gtk_list_store_append(store, &iter);
        gtk_list_store_set(store, &iter, 0, id, 1, name, 2, param, 3, quantity, -1);
        step = sqlite3_step(res);
    }
...
void add_order_to_table(int id, const char *customer, const char *name, int value, int amount, int sum,
    ...
    Product new_order;
    new_order.id = id;
    strncpy(new_order.customer, customer, 100);
    strncpy(new_order.name, name, 100);
    new_order.value = value;
    new_order.amount = amount;
    new_order.sum = sum;
    strncpy(new_order.pay_type, pay_type, 10);
    strncpy(new_order.date, date, 100);
    ...
    sqlite3_stmt *res;
    sqlite3_prepare_v2(db, sql, -1, &res, 0);
    sqlite3_bind_int(res, 1, new_order.id);
    sqlite3_bind_text(res, 2, new_order.customer, -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(res, 3, new_order.name, -1, SQLITE_TRANSIENT);
    sqlite3_bind_int(res, 4, new_order.value);
    sqlite3_bind_int(res, 5, new_order.amount);
    sqlite3_bind_int(res, 6, new_order.sum);
    sqlite3_bind_text(res, 7, new_order.pay_type, -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(res, 8, new_order.date, -1, SQLITE_TRANSIENT);
